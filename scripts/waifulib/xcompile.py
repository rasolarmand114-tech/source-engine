...
ANDROID_NDK_SYSROOT_FLAG_MAX = 19
...
def _ensure_executable(path):
	'''
	Make sure a toolchain binary actually has its executable bit set.

	NDKs fetched from an official tarball/zip always preserve the
	executable bit on their prebuilt binaries. NDKs that get mirrored
	through a plain "git clone" (as some CI setups do, instead of
	downloading the real archive) frequently lose that bit: the blob
	ends up committed as mode 100644, and git checks it out as a
	regular non-executable file. waf's find_program() then locates the
	file but refuses to use it ("Program [...] is not executable").

	This is a cheap, harmless self-heal: if the file exists but isn't
	executable, flip the bit. It's a no-op for correctly-permissioned
	NDKs.
	'''
	...
	try:
		...
		Logs.warn(
			'Could not fix executable bit on %s: %s' % (path, e)
		)
...
class Android:
	...
	def __init__(self, ctx, arch, toolchain, api):
		...
		for i in ANDROID_NDK_ENVVARS:
			...
			ctx.fatal(
				'Set %s environment variable pointing to the root of Android NDK!' %
				' or '.join(ANDROID_NDK_ENVVARS)
			)

		# NDK r10e did not have source.properties.
				...
				for line in ndk_props_file.readlines():
					...
					if 'Pkg.Revision' in trimed_tokens:
						try:
							...
							ctx.fatal('Unable to determine NDK revision')
		...
		if self.api < ANDROID_NDK_API_MIN[self.ndk_rev]:
			...
			Logs.warn(
				'API level automatically was set to %d due to NDK support'
				% self.api
			)

		if self.is_arm64() or self.is_amd64():
			if self.api < ANDROID_64BIT_API_MIN:
				self.api = ANDROID_64BIT_API_MIN
				Logs.warn(
					'API level for 64-bit target automatically was set to %d'
					% self.api
				)

	def is_host(self):
		'''
		Checks if we are using the host compiler.
		For modern NDKs this means the LLVM compiler shipped with the NDK.
		'''
	...
	def is_x86(self):
		'''
		Checks if selected architecture is 32-bit x86.
		'''
	...
	def is_clang(self):
		'''
		Checks if selected toolchain is Clang.
		'''
		return self.clang
	...
	def gen_host_toolchain(self):
		# With the host toolchain we use the Linux x86_64 NDK package.
		if self.is_host():
			return 'linux-x86_64'
	...
	def gen_gcc_toolchain_path(self):
		...
		Legacy NDKs retain their old GCC/Clang layout.
		'''
		...
		if self.ndk_rev >= 27:
			...
			toolchain_folder = '%s-%s' % (
				self.ndk_triplet(toolchain_folder=True),
				toolchain
			)
	...
	def gen_toolchain_path(self):
		'''
		Return the compiler prefix.

		For modern LLVM NDKs this becomes for example:
		...
		if self.is_clang():
			triplet = '%s%d-' % (
				self.ndk_triplet(llvm_toolchain=True),
				self.api
			)
		else:
	...
	def cc(self):
		...
		if self.is_host():
			return 'clang --target=%s%d' % (
				self.ndk_triplet(),
				self.api
			)

		return self.gen_toolchain_path() + (
			'clang' if self.is_clang() else 'gcc'
		)
	...
	def libsysroot(self):
		...
		path = 'platforms/android-%s/arch-%s' % (
			self.api,
			arch
		)
	...
	def cflags(self, cxx=False):
		...
		# Old NDKs need an explicit sysroot.
		...
		#
		# when invoked with the target triple.

		cflags += [
			'-I%s' % i
			for i in self.system_stl()
		]
		...
		if cxx and not self.is_clang() and self.toolchain not in [
			'4.8',
			'4.9'
		]:
			cflags += [
		...
		if self.is_arm():
			...
			if self.arch == 'armeabi-v7a':
				cflags += [
					...
					'-mcpu=cortex-a7',
					'-mtune=cortex-a7',
					'-DHAVE_EFFICIENT_UNALIGNED_ACCESS',
					'-DVECTORIZE_SINCOS'
				]
	...
	def ldflags(self):
		...
		# GCC runtime is only relevant for the legacy GCC toolchains.
		...
		elif self.is_clang() or self.is_host():
			ldflags += [
				'-stdlib=libstdc++'
			]
...
def options(opt):
	...
	android.add_option(
		...
		help=(
			...
			'example: --android=armeabi-v7a-hard,4.9,21'
		)
	)
...
def configure(conf):
	...
	if conf.options.ANDROID_OPTS:
		...
		if len(values) != 3:
			conf.fatal(
				'Invalid --android paramater value!'
			)
		...
		try:
			...
			conf.fatal(
				'Invalid Android API level: %s' %
				values[2]
			)

		conf.android = android = Android(
			conf,
			values[0],
			values[1],
			api
		...
		if android.ndk_rev >= 27:
			...
			# sources/cxx-stl/gnu-libstdc++ tree.

			# Use the shared libc++ runtime. The NDK clang/lld
			# driver resolves it from the Android target sysroot.
			conf.env.LDFLAGS += [
				'-lc++_shared'
			]
...
compiler_cxx_configure = getattr(
	compiler_cxx,
	'configure'
)

compiler_c_configure = getattr(
	compiler_c,
	'configure'
)
